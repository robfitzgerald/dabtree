...
object GenericPedrosoReiSynchronization {
  ...
  def synchronize[G[_] : Monad : Foldable, S, A, V : Numeric](
    payloads: G[(BanditParent[S, A, V], Option[UCBPedrosoReiGlobals[S,A,V]])],
    Sampler: UCBPedrosoReiSampler[S,A,V]
  )(implicit m: Monoid[UCBPedrosoReiGlobalState[S,A,V]]): G[(BanditParent[S, A, V], Option[UCBPedrosoReiGlobals[S,A,V]])] = {
    ...
    payloads.map{ payload =>
      ...
      if (parent.searchState != SearchState.Activated) payload
        ...
        val updatedGlobalsOption: Option[UCBPedrosoReiGlobals[S,A,V]]= globalsOption.map { _.copy(state = bestGlobalState) }
        val updatedParent =
