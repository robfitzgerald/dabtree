package com.github.robfitzgerald.dabtree.local
...
object Objective {
  ...
  case class Minimize[V: Order](optimalBounds: V, badBounds: V)(implicit val numeric: Numeric[V]) extends Objective[V] {
    require(numeric.compare(optimalBounds, badBounds) < 0, s"Minimize objective should have a 'optimalBounds' < 'badBounds' (since min is optimal), but instead $optimalBounds >= $badBounds.")
    ...
    override def isMoreOptimalThan(a: V, b: V): Boolean = numeric.compare(a, b) < 0
  }

  case class Maximize[V: Order](badBounds: V, optimalBounds: V)(implicit val numeric: Numeric[V]) extends Objective[V] {
    require(numeric.compare(optimalBounds, badBounds) > 0, s"Maximize objective should have a 'optimalBounds' > 'badBounds' (since max is optimal), but instead $optimalBounds <= $badBounds.")
