package com.github.robfitzgerald.dabtree.local.mctsstats.immutable
  ...
  def empty[V : Numeric](objective: Objective[V]): MCTSStatsImmutableImpl[V] = {
    objective match {
      case Objective.Minimize(optimalBounds, badBounds) =>
        val midpoint: V = (badBounds + optimalBounds) / 2
        MCTSStatsImmutableImpl[V](badBounds, optimalBounds, midpoint, 0)
      case Objective.Maximize(optimalBounds, badBounds) =>
        val midpoint = (badBounds + optimalBounds) / 2
        MCTSStatsImmutableImpl[V](optimalBounds, badBounds, midpoint, 0)
    }
  }
}
